...
var color = Color.fromHex("d9ff03") as Color;

var Skitso = MaterialSystem.getMaterialBuilder().setName("Skitso").setColor(color).build();

Skitso.registerParts(["gear", "plate", "nugget", "ingot", "rod", "dust"] as string[]);

var molten = Skitso.registerPart("molten").getData();
molten.addDataValue("temperature", "400");
molten.addDataValue("luminosity", "10");

var block = Skitso.registerPart("block").getData();
block.addDataValue("hardness", "5");
block.addDataValue("resistance", "30");
block.addDataValue("harvestTool", "pickaxe");

var ore = Skitso.registerPart("ore").getData();
...
var armor = Skitso.registerPart("armor").getData();
