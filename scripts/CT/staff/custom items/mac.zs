...
var color = Color.fromHex("7DF9FF") as Color;

var Warlock = MaterialSystem.getMaterialBuilder().setName("Warlock").setColor(color).build();

Warlock.registerParts(["gear", "plate", "nugget", "ingot", "rod", "dust"] as string[]);

var molten = Warlock.registerPart("molten").getData();
molten.addDataValue("temperature", "400");
molten.addDataValue("luminosity", "10");

var block = Warlock.registerPart("block").getData();
block.addDataValue("hardness", "5");
block.addDataValue("resistance", "30");
block.addDataValue("harvestTool", "pickaxe");

var ore = Warlock.registerPart("ore").getData();
...
var armor = Warlock.registerPart("armor").getData();
