...
var color = Color.fromHex("000000") as Color;

var Zephyrus = MaterialSystem.getMaterialBuilder().setName("Zephyrus").setColor(color).build();

Zephyrus.registerParts(["gear", "plate", "nugget", "ingot", "rod", "dust"] as string[]);

var molten = Zephyrus.registerPart("molten").getData();
molten.addDataValue("temperature", "400");
molten.addDataValue("luminosity", "10");

var block = Zephyrus.registerPart("block").getData();
block.addDataValue("hardness", "5");
block.addDataValue("resistance", "30");
block.addDataValue("harvestTool", "pickaxe");

var ore = Zephyrus.registerPart("ore").getData();
...
var armor = Zephyrus.registerPart("armor").getData();
