...
var color = Color.fromHex("F9F6EE") as Color;

var whitetiger  = MaterialSystem.getMaterialBuilder().setName("whitetiger").setColor(color).build();

whitetiger.registerParts(["gear", "plate", "nugget", "ingot", "rod", "dust"] as string[]);

var molten = whitetiger .registerPart("molten").getData();
molten.addDataValue("temperature", "400");
molten.addDataValue("luminosity", "10");

var block = whitetiger .registerPart("block").getData();
block.addDataValue("hardness", "5");
block.addDataValue("resistance", "30");
block.addDataValue("harvestTool", "pickaxe");

var ore = whitetiger.registerPart("ore").getData();
...
var armor = whitetiger.registerPart("armor").getData();
